...
Parser.prototype.parse = function(sentence) {
	...
	var row = new Chart.ChartRow(new Grammar.Rule(Parser.GAMMA_SYMBOL, ['S']), 0, 0)
  	charts[0].add_row(row);
	
	// we go word by word
	for (var i = 0; i < charts.length; i++) {
    	var chart = charts[i];
    	this.prescan(chart, sentence, i); // scan current input

    	var length = chart.length;
    	var old_length = -1;
    	while (old_length != length) {
			this.predict(chart, i);
			this.complete(charts, chart, i)
			old_length = length;
			length = chart.length;
		}
	}
	...
	return {
		valid: res,
		complete_parses: complete_parses
	}
}
...
Parser.prototype.prescan = function(chart, sentence, position) {
	var word = sentence.words[position-1];
	if (word) {
		var rules = [];
...
Parser.prototype.complete = function(charts, chart, position) {
	for (var i = 0; i < chart.rows.length; i++) {
		var row = chart.rows[i];
		if (row.is_complete()) {
			var completed = row.rule.lhs;
			for (var n = 0; n < charts[row.start].rows.length; n++) {
				var r = charts[row.start].rows[n];
